...
**[AzureDevOps.WikiPdfExport.exe](https://github.com/MaxMelcher/AzureDevOps.WikiPdfExport/releases)*- (~30MB)
...
Headers and footers can be added to the document by the `--header-*` and `--footer*` template arguments respectfully.
In header and footer string supplied the following variables will be substituted. See [PDF options from puppeteer](https://pptr.dev/#?product=Puppeteer&show=api-pagepdfoptions).
...
run `AzureDevOps.WikiPdfExport -p .` to generate your PDF.
